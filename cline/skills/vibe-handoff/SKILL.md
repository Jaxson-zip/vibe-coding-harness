---
name: vibe-handoff
description: Generate a handoff package for switching to another AI tool (e.g. Codex→Claude Code, Cline→Cursor). Produces a concise copy-paste bundle so the next tool can resume without losing context. Use when context is filling up, when doing cross-tool review, or when switching tools intentionally. Triggers on: "handoff", "switch tool", "交接", "换工具", "/vibe-handoff".
---

跨工具交接命令:生成一份精简交接包,让另一个 AI 工具(Claude Code / Codex / Cursor / Cline)能无缝接上当前任务,不依赖会话记忆。
...
### 你需要先读这些文件(按顺序)
1. AGENTS.md — 项目行为规范(尤其是第 1.1 防发散、第 9 交接、第 9.1 心跳、第 10 角色协作)
2. docs/CURRENT.md — 当前任务状态(必须第一个读)
3. docs/REQ-{today}-{NN}-*.md — 当前 feature 需求
4. docs/DEV-{today}-{NN}-*.md — 技术方案 + task 清单(只读当前 task 段)
5. vibe-coding-harness 的 vibe-role skill 内 docs/<role>.md — 你的角色定义(或项目内 roles/<role>.md)
...
- 心跳式更新:每完成一个动作立即更新 docs/CURRENT.md,防被动中断。
...
| 当前角色 | 推荐换到 | 理由 |
|---|---|---|
| backend/frontend(Cline/OpenCode/Codex 写的) | Claude Code | 交叉审查补盲区 |
| reviewer | 与实现者不同的工具 | 强制交叉 |
| tester | 任意 | 测试用例可移植 |
| devops | 与实现者同 | 部署配置不需要交叉 |
| pm/designer/architect | 任意(用最强模型) | 文档质量优先 |

## 铁律
- **交接包要精简**:只列必读文件 + 当前状态 + 铁律,不要把整个对话历史塞进去。
- **新工具必须读 CURRENT.md**:这是唯一真相源,不读等于没交接。
- **交接不等于完成**:交接后必须更新 CURRENT.md 记录交接事实,否则下次会断档。
- **被动中断不要用本命令**:如果是额度爆/断网/崩溃,新工具第一句话跑 `/vibe-resume`,不需要交接包。
